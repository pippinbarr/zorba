	...
	public class ZorbaState extends FlxState
	{
		...
		public override function update():void {
			...
			if (_addDancers) {
				...
				if (_uis[0].playing()) {
					...
					if (_time > Cookie.longestDanceVsZorba) Cookie.longestDanceVsZorba = Math.floor(_time);
				}
		...
		private function uiKeyDown(e:KeyboardEvent):void {
			if (e.keyCode == Keyboard.Q) {
				Assets._failSound.play(true);
				FlxG.stage.removeEventListener(KeyboardEvent.KEY_DOWN,uiKeyDown);
				Cookie.zorbaLosses++; // Automatically lose if you quit during dancing process
